...
async function buildMentorContext(userId, { message = '', mentorMode = 'general', action = '', sourcesOverride = null } = {}) {
  ...
  if (sources.includes('goals')) {
    ...
    blocks.push(`GOALS\n${buildGoalsBlock(loaded.goals)}`)
    const primaryGoal = loaded.goals.find((item) => item.status === 'active') || loaded.goals[0]
    if (primaryGoal) {
  ...
  if (sources.includes('tasks')) {
    ...
    blocks.push(`TASKS\n${buildTasksBlock(loaded.tasks)}`)
  }
  if (sources.includes('roadmaps')) {
  ...
  if (sources.includes('career') || sources.includes('skills')) {
