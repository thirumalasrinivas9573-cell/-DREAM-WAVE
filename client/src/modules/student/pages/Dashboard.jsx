...
import { goalApi, taskApi, dailyApi } from '../../shared/services/api'
...
import { StreakFlame, XPRing } from '../../shared/components/Gamification'

const stagger = { animate: { transition: { staggerChildren: 0.06 } } }
const fadeUp  = { initial: { opacity: 0, y: 18 }, animate: { opacity: 1, y: 0, transition: { duration: 0.32 } } }

const QUICK = [
  { to: '/student/goals',        icon: '🎯', label: 'Goals',         color: '#8B5CF6', desc: 'Set & track targets'   },
  { to: '/student/roadmap',      icon: '🗺️', label: 'Roadmap',       color: '#6366F1', desc: 'Career intelligence'   },
  { to: '/student/learn',        icon: '🎓', label: 'AI Studio',     color: '#A855F7', desc: 'Cinematic lessons'     },
  { to: '/student/tasks',        icon: '⚒️', label: 'Learning',      color: '#10B981', desc: 'Daily skill engine'    },
  { to: '/student/mentor',       icon: '🤖', label: 'AI Mentor',     color: '#C084FC', desc: 'Personal guidance'     },
  { to: '/student/reports',      icon: '📊', label: 'R&D Reports',   color: '#818CF8', desc: 'Deep career research'  },
  { to: '/student/analytics',    icon: '📈', label: 'Analytics',     color: '#22D3EE', desc: 'Live learning signals' },
  { to: '/student/certificates', icon: '🏅', label: 'Certificates',  color: '#FBBF24', desc: 'Issue credentials'     },
  { to: '/student/resume',       icon: '📄', label: 'Resume',        color: '#A78BFA', desc: 'ATS-friendly builder'  },
  { to: '/student/books',        icon: '📚', label: 'Books',         color: '#34D399', desc: 'Curated library'       },
  { to: '/student/community',    icon: '💬', label: 'Community',     color: '#9333EA', desc: 'Learn together'        },
]

const JOURNEY = [
  { icon: '🎯', label: 'Set Goal',     route: '/student/goals',   status: 'done'    },
  { icon: '🗺️', label: 'Roadmap',      route: '/student/roadmap', status: 'active'  },
  { icon: '🎓', label: 'Learn',        route: '/student/learn',   status: 'pending' },
  { icon: '❓', label: 'Quiz',         route: '/student/tasks',   status: 'pending' },
  { icon: '⚒️', label: 'Practice',    route: '/student/tasks',   status: 'pending' },
  { icon: '📊', label: 'Assess',       route: '/student/reports', status: 'pending' },
  { icon: '📄', label: 'Resume',       route: '/student/resume',  status: 'pending' },
  { icon: '🚀', label: 'Career',       route: '/student/roadmap', status: 'pending' },
]
...
export default function Dashboard() {
  ...
  const STATS = [
    { icon: '🎯', label: 'Active Goals',  value: activeGoals.length,  color: '#8B5CF6', bg: 'rgba(139,92,246,0.1)',  to: '/student/goals'  },
    { icon: '⚒️', label: 'Tasks Pending', value: pendingTasks.length, color: '#10B981', bg: 'rgba(16,185,129,0.1)',  to: '/student/tasks'  },
    { icon: '✅', label: 'Completed',      value: doneTasks.length,    color: '#A855F7', bg: 'rgba(168,85,247,0.1)',  to: '/student/tasks'  },
    { icon: '📖', label: 'Done Today',     value: todayDone,           color: '#6366F1', bg: 'rgba(99,102,241,0.1)',  to: '/student/tasks'  },
  ]
  ...
  return (
    <StudentLayout>
      ...
      >
        <NeuralBg nodeCount={28} color="#8B5CF6" opacity={0.28} />
        ...
        <motion.div variants={fadeUp} initial="initial" animate="animate">
          <div className="card" style={{ height: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3>🎯 Active Goals</h3>
              <Link to="/student/goals" className="btn btn-secondary btn-sm">View all</Link>
            </div>
            {loading
              ...
              : activeGoals.length === 0
                ? (
                  <div className="empty-state" style={{ padding: '24px 0' }}>
                    ...
                    <Link to="/student/goals" className="btn btn-primary btn-sm" style={{ marginTop: 10, textDecoration: 'none' }}>Create Goal</Link>
                  </div>
                )
        ...
        <motion.div variants={fadeUp} initial="initial" animate="animate">
          <div className="card" style={{ height: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <Link to="/student/tasks" className="btn btn-secondary btn-sm">View all</Link>
            </div>
            {loading
              ...
              : pendingTasks.length === 0
                ? (
                  <div className="empty-state" style={{ padding: '24px 0' }}>
                    ...
                    <Link to="/student/tasks" className="btn btn-primary btn-sm" style={{ marginTop: 10, textDecoration: 'none' }}>Add Task</Link>
                  </div>
                )
                : pendingTasks.map((t, i) => {
                  ...
                  return (
                    <Link to="/student/tasks" key={t._id} style={{ textDecoration: 'none' }}>
                      <motion.div
        ...
        <motion.div variants={fadeUp} initial="initial" animate="animate">
          <div className="card" style={{ height: '100%', background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(13,13,23,0.9))', border: '1px solid var(--border-purple)' }}>
            ...
            <div style={{ marginTop: 14 }}>
              <Link to="/student/mentor" className="btn btn-secondary btn-sm" style={{ width: '100%', textDecoration: 'none', justifyContent: 'center' }}>
                🤖 Ask AI Mentor
              </Link>
            </div>
          </div>
